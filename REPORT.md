...
| Area | Hasil |
|---|---|
| Suite PHP (Unit + Feature) | **287 passed** (1309 assertions) |
...
- **Fase 7.2** (CSP): disesuaikan tanpa melonggarkan origin pihak ketiga; `object-src 'self'` untuk dokumen same-origin.
...
- Biarkan `node_modules/` dan `results/` di bawah `scripts/qa/` tidak di-commit (`.gitignore` lokal).
